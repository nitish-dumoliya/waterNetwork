...
nlp_ampl.read_data(f"/home/nitishdumoliya/waterNetwork/data/{data_list[0]}.dat")
nlp_ampl.option["solver"] = "knitro"
nlp_ampl.option["knitro_options"] = "outlev = 1 ms_enable 1  ms_maxsolves 1 mip_multistart 1 "
...
for (i,j,k) in l_nlp.keys():
    if l_nlp[i,j,k]>=1:
        nlp_ampl.eval(f"s.t. fix_length_{i}_{j}_{k}: l[{i},{j},{k}]={l_nlp[i,j,k]};")
