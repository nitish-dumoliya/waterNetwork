...
class WaterNetworkOptimizer:
    ...
    def plot_graph(self, super_source_out_arc=None, best_arc=None, current_cost = None, iteration = 1, edge_weights= None, h = None, D = None, arc = None):
        # self.network_graph = nx.DiGraph()
...
#         print("total_cost:", self.ampl.get_objective("total_cost").value())
    ...
    def iterate_arc(self, iteration, best_acyclic_flow, improved, current_cost, l, q, h, best_arc, super_source_out_arc):
        ...
        dual_value = con2.get_values('dual').to_dict()
        ...
        for node in indegree_2_or_more:
            for (u, v) in list(self.network_graph.in_edges(node)):
                if (u,v) in arc_set:
                    print(f"dual_value[{u},{v}]", dual_value[(u,v)])
                else:
                    print(f"dual_value[{u},{v}]", dual_value[(v,u)])
                    
        # var = self.ampl.get_variable('q')
        # var_h_value = var.get_values('dual').to_dict()
        # print(var_h_value)
        
        con6 = self.ampl.get_constraint('con6')
        dual_value = con6.get_values('dual').to_dict()
        # print("dual_value:",dual_value)
        for node in indegree_2_or_more:
            print(f"dual_value[{node}]", dual_value[node])
    ...
    def iterate_acyclic_flows(self):
        ...
        D = self.ampl.getParameter('D').getValues().to_dict()
        self.plot_graph(super_source_out_arc, best_arc, current_cost, 0, q, h, D, (0,0))
    ...
    def solve(self):
        with self.suppress_output():
            ...
            self.ampl.option["solver"] = "ipopt"
            self.ampl.set_option("ipopt_options", "outlev = 0 expect_infeasible_problem = yes bound_push = 0.001 bound_frac = 0.001 warm_start_init_point = yes least_square_init_duals = no ")   #max_iter = 1000
