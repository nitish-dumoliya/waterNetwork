...
for (i,j) in q_lp.keys():
    dualVar[i,j] = lp_ampl.get_constraint('con1')[i,j].dual()

sortedDualVar = sorted(dualVar.items(), key=lambda x:x[1], reverse=True)
...
flow_ampl.eval(f"s.t. fix_q_del3_20: q_del[3,20]={delta};")
flow_ampl.eval(f"s.t. fix_q_del3_19: q_del[3,19]={-delta1};")
