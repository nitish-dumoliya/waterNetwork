...
s.t. con2{(i,j) in arcs}: 
    h[i] - h[j] = (0.001^1.852)*(q[i,j] * abs(q[i,j])^0.852) * omega * L[i,j]*(sum{k in pipes} (X[i,j,k] / (R[k]^1.852 * (d[k]/1000)^4.87)));
...
subject to flow_bound1 {(i,j) in arcs}:
    -sum{k in nodes diff Source} D[k] <= q[i,j] <= sum {k in nodes diff Source} D[k];
