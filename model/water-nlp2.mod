...
#subject to con2{(i,j) in arcs}: 
#     h[i] - h[j]  = (q[i,j])^3 *((((q[i,j])^2 + 1e+6 * eps[i,j] )^0.426) /((q[i,j])^2 + 0.426 * 1e+6 *eps[i,j])) * (1000^3.018)  * sum{k in pipes} (omega * l[i,j,k] / ( (R[k]^1.852) * (d[k])^4.87));

subject to con2{(i,j) in arcs}: 
     h[i] - h[j]  = (q[i,j]/1000)^3 *((((q[i,j]/1000)^2 + eps[i,j])^0.426) /((q[i,j]/1000)^2 + 0.426 *eps[i,j])) * sum{k in pipes} (omega * l[i,j,k] / ( (R[k]^1.852) * (d[k]/1000)^4.87));
...
#subject to eps_selection{(i,j) in arcs}: 0.07508*((omega / ( (R[1]^1.852) * (d_min)^4.87))) * eps[i,j]^0.926 = 1e-6;
#subject to eps_selection{(i,j) in arcs}: eps[i,j] = ((1e-6)/(0.07508*((omega / ( (R[1]^1.852) * (d_min)^4.87)))))^(1/0.926);
