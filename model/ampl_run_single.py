...
class WaterNetworkSolver:
    ...
    def constraint_violations(self, q_values, h_values, X_values, solver):
        ...
        for (i, j) in q_values.keys():
            ...
            alpha_rhs = sum(10.67 * X_values[i, j, k] / ((self.R[k] ** 1.852) * ((self.d[k] / 1000) ** 4.87)) for k in self.pipes)
            original_rhs =  (0.00000277971326776) * q_values[i, j] * (abs(q_values[i, j])) ** 0.852 * self.L[i,j] * alpha_rhs
            
            # Approximated constraint value
            approx_rhs = (0.00000277971326776)*(q_values[i, j]**3 * ((q_values[i, j]**2 + 0.0001) ** 0.426)/(q_values[i,j]**2 + 0.000426)) * self.L[i,j] * alpha_rhs
        ...
        for (i,j) in self.arcs:
            con3_rhs = 1
            con3_lhs = sum(X_values[i,j,k] for k in self.pipes) 
    ...
    def solve_ipopt(self):
        ...
        X_init = self.ampl.getVariable('X').getValues().to_dict()
        ...
        self.constraint_violations(q_init, h_init, X_init, "ipopt")
        ...
        X_sol = self.ampl.get_variable('X').get_values().to_dict()
        ...
        for idx in X_sol.keys():
            self.X_init[idx] = X_sol[idx]
        print("*******************************************************************************\n")
    ...
    def second_solve(self):
        ...
        X = self.ampl.get_variable('X').get_values().to_dict()

        self.constraint_violations(q, h, X, self.solver_name)
