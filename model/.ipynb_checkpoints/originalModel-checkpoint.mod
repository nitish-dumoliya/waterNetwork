...
subject to con1{j in nodes}: 
    sum{i in nodes : (i,j) in arcs }q1[i,j] -  sum{i in nodes : (j,i) in arcs}q1[j,i] =  -D[j]
;
subject to con2{(i,j) in arcs}: 
    h[i] - h[j] = (q1[i,j] * abs(q1[i,j])^0.852) * (0.001^1.852) * sum{k in pipes } omega * l[i,j,k] / ( (R[k]^1.852) * (d[k]/1000)^4.87)
;

# s.t. con3{(i,j) in arcs }: (if -0.1<=q[i,j]<=0.1  then 
...
# subject to flow_bound{(i,j) in arcs}: 
#   q[i,j] <= vmax[i,j]*(3.14/4)*(L[i,j]/(sum{k in pipes} l[i,j,k]/(d[k])**4.87))**(2/4.87)
#;

# subject to flow_bound_{(i,j) in arcs}: 
