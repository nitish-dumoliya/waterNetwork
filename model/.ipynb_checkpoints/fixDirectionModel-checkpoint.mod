...
var h{nodes};	            # Head
var x{arcs} binary;                # Direction of flow (if x = 1 then direction is positive or if x = 0 then direction is negative) 
...
subject to con7{(i,j) in arcs}:
    (-sum{k in nodes diff Source} D[k])*(1-x[i,j]) <= q[i,j];
