...
param eps default 1e-2;  # Small smoothing parameter
...
subject to con2{(i,j) in arcs}:
     h[i] - h[j]  = ((q[i,j] * abs(q[i,j])*(abs(q[i,j])+eps)^0.852) / (abs(q[i,j]) + 0.852*eps)) * sum{k in pipes} (omega * l[i,j,k] / ( (R[k]^1.852) * (d[k])^4.87));
