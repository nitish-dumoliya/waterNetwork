...
class WaterNetworkSolver:
    ...
    def constraint_violations(self, q_values, h_values, l_values, epsilon, solver):
        ...
        for (i, j) in q_values.keys():
            ...
            original_rhs = (0.001**1.852)*q_values[i, j] * (abs(q_values[i, j])) ** 0.852 * alpha_rhs 
            #original_rhs =  q_values[i, j] * (abs(q_values[i, j])) ** 0.852 * alpha_rhs
            
            # Approximated constraint value
            approx_rhs = (0.001**1.852)*(q_values[i, j]**3 * ((q_values[i, j]**2 + 1e+6 * epsilon[i,j]) ** 0.426)/(q_values[i,j]**2 + 0.426*1e+6*epsilon[i,j])) * alpha_rhs
    ...
    def second_solve(self):
        ...
        #self.ampl.eval("s.t. q_12: z[1,2]=1;")
        #self.ampl.eval("s.t. q_23: z[2,3]=1;")
        #self.ampl.eval("s.t. q_24: z[2,4]=1;")
        #self.ampl.eval("s.t. q_35: z[3,5]=1;")
        #self.ampl.eval("s.t. q_45: z[4,5]=1;")
        #self.ampl.eval("s.t. q_46: z[4,6]=1;")
        #self.ampl.eval("s.t. q_67: z[6,7]=1;")
        #self.ampl.eval("s.t. q_75: z[7,5]=1;")
        #
