...
var h{nodes};	            # Head
var dia{arcs};		   # Diameter of each arcs
var x{arcs} binary;

#****************************************OBJECTIVE****************************************#
#minimize total_cost : sum{(i,j) in arcs} sum{k in pipes}l[i,j,k]*C[k];	
minimize total_cost : sum{(i,j) in arcs} dia[i,j]^4.87 * L[i,j];	
...
subject to con2{(i,j) in arcs}: 
    (h[i] - h[j])*x[i,j] = (q[i,j] * abs(q[i,j])^0.852) * (0.001^1.852) * (omega * L[i,j] / ( (130^1.852) * (dia[i,j])^4.87))
;
...
subject to con8{(i,j) in arcs}:
    q[i,j] <= x[i,j]*(sum{k in nodes diff Source} D[k])
;

subject to con9{(i,j) in arcs}:
    0.0254 <= dia[i,j] <= 0.6096;
#*******************************************************************************************#
