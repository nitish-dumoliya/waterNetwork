...
minimize total_cost : 
    sum{(i,j) in arcs} (sum{k in pipes} C[k]*l[i,j,k])*x[i,j];

#**********************************CONSTRAINTS***************************************#

s.t. con1 {(i,j) in arcs}:
    (sum {k in pipes} l[i,j,k] - L[i,j])*x[i,j] = 0;
...
s.t. con3{(i,j) in arcs}: 
    (h[i] - h[j])*x[i,j] = (0.001^1.852)*(q[i,j] * abs(q[i,j])^0.852) * omega *(sum{k in pipes} (l[i,j,k] / (R[k]^1.852 * (d[k]/1000)^4.87)));
...
s.t. con4 {(i,j) in arcs, k in pipes}:
    l[i,j,k] <= L[i,j];

s.t. con5 {k in Source}: h[k] = E[k];

s.t. con6 {i in nodes diff Source}: 
    (h[i] - E[i] - P[i])*v[i] >=0;
