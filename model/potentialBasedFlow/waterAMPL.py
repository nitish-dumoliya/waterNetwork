...
class WaterNetworkOptimizer:
    ...
    def constraint_violations(self, q_values, h_values, l_values, R_values, d_values, pipes, epsilon):
       ...
       for (i, j) in q_values.keys():
           ...
           approx_rhs1 = (q_values[i,j]*(abs(q_values[i,j])+0.148*epsilon[i,j]) /(abs(q_values[i,j])+epsilon[i,j])**0.148)


           approx_rhs2 = q_values[i, j] * ((abs(q_values[i, j]) + epsilon[i,j]) ** 0.852) * \
                        (abs(q_values[i, j]) / (abs(q_values[i, j]) + 0.852 * epsilon[i,j])) 
 
           approx_rhs3 = (q_values[i, j] * ((abs(q_values[i, j]) + epsilon[i,j]) ** 0.852) * \
                        (abs(q_values[i, j]) / (abs(q_values[i, j]) + 0.852 * epsilon[i,j])) +\
                       (0.063048*(epsilon[i,j])**2) * q_values[i,j]/((abs(q_values[i,j])+epsilon[i,j])**1.148)) 
        ...
        for node in  self.sorted_nodes:
            ...
            def objective(params):
               ...
               if (u,v) in self.arcs:
                   ...
                   self.ampl.eval(f"s.t. flow_direction1{u}_{v}: q[{v}, {u}]>=1e-9;")
            ...
            with self.suppress_output():
                best_params, best_cost = pso(objective, lb, ub, swarmsize=5, maxiter=5)
    ...
    def objective(self, params):
        ...
        ipopt_options = f"outlev=0 expect_infeasible_problem=yes tol = 1e-9 bound_relax_factor=0  bound_push={bound_push} bound_frac={bound_frac}  warm_start_init_point = yes nlp_scaling_method=none "
    ...
    def solve(self):
        ...
        self.ampl.eval("option solution_round 5;")
    ...
    def run(self):
        ...
        best_params, best_cost = pso(self.objective, lb, ub, swarmsize=3, maxiter=3)
        self.bound_push , self.bound_frac = best_params
        #self.bound_push , self.bound_frac = (0.01, 0.01)
        ...
        #Q_max = self.ampl.getParameter('Q_max')
...
if __name__ == "__main__":
    ...
    input_data_file = f"/home/nitishdumoliya/waterNetwork/data/minlplib_data/{data_list[(data_number)]}.dat"
    print("Water Network:", data_list[(data_number)],"\n")

    optimizer = WaterNetworkOptimizer("../water-nlp1.mod", input_data_file, data_number, data_list)
