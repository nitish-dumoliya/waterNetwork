...
class WaterNetworkOptimizer:
    ...
    def iterate_arc(self,best_acyclic_flow, improved, current_cost, l, q, h):
        improved = False
        # self.total_cost = current_cost
        self.network_graph = best_acyclic_flow.copy()
        print("current network arcs direction: ",self.network_graph.edges())
        # self.plot_graph()

        for u, v in list(self.network_graph.edges()):  # Use a list to avoid modifying the graph during iteration
            if (u,v) not in self.fix_arc(data_number):
                # Attempt to reverse the direction of the arc (u, v) to (v, u)
                self.network_graph.remove_edge(u, v)
                self.network_graph.add_edge(v, u)  # Reversing the arc direction

                # print((u,v),nx.is_directed_acyclic_graph(self.network_graph))
                # Check if the new graph is still acyclic
                # print(f"Attempt to reverse the direction of the arc ({u}, {v}) to ({v}, {u})")
                if nx.is_directed_acyclic_graph(self.network_graph):
                    check_in_degree = self.check_incoming_arcs()
                    if check_in_degree:
                        ...
                        self.solve()  # Solves the flow optimization problem
                        if self.solve_result == "solved":
                            
                            if self.total_cost < current_cost:
                                print("Arc", (u,v),"Acyclic: Yes Previous solution: ", current_cost, "New_solution: ", self.total_cost, "Solve_result: ", self.solve_result, "Improved: Yes")
                                # print("solve_result: ",self.solve_result)
                                # If the new cost is lower, update the current best configuration
                                # print("Previous solution:", current_cost)
                                current_cost = self.total_cost
                                improved = True
                                # print(f"Improved solution found: {current_cost}")
                                ...
                                # print(" ")
                                # Restore the original direction of the arc if no improvement or if the graph becomes cyclic
                                print("Arc", (u,v),"Acyclic: Yes Previous solution: ", current_cost, "new_solution: ", self.total_cost, "solve_result: ", self.solve_result, "Improved: No")
                                self.network_graph.remove_edge(v, u)
                                self.network_graph.add_edge(u, v)  # Restore the original arc direction
                        elif self.solve_result == "infeasible":
                            # print("Solve status: ",self.solve_result)
                            print("Arc", (u,v),"Acyclic: Yes Previous solution: ", current_cost, "new_solution: ", self.total_cost, "solve_result: ", self.solve_result)
                            # self.generate_random_acyclic_graph()
                    else:
                        print("Arc", (u,v),"Acyclic: No")
                        # print(f"Restoring original direction.")
                        # print(" ")
                        # Restore the original direction of the arc if no improvement or if the graph becomes cyclic
                        self.network_graph.remove_edge(v, u)
    ...
    def iterate_acyclic_flows(self):
        ...
        # print(self.fix_arc(data_number))
        iteration = 1
        while improved:
            print("Iteration :",iteration)
            best_acyclic_flow, improved, current_cost, l, q, h = self.iterate_arc(best_acyclic_flow, improved, current_cost, l, q, h)
            if current_cost == 10e+14:
                print("search optimal acyclic network")
                self.load_model()
                self.generate_random_acyclic_graph()
                self.update_model()
                self.solve()
        ...
        # print(f"Final best solution: {current_cost}")
        self.best_flow = list(best_acyclic_flow.edges(data=False))  # Store the best flow configuration
