...
param Rough{arcs} default 130;
#****************************************VARIABLES****************************************#
var X{arcs,pipes} binary ;          # Binary variable for each commercial pipe for each arc/link
...
minimize total_cost : sum{(i,j) in arcs} L[i,j] *(sum{k in pipes} X[i,j,k]*C[k]);	

#****************************************CONSTRAINTS**************************************#
s.t. con1{j in nodes }: sum{i in nodes : (i,j) in arcs }(q1[i,j] - q2[i,j]) -  sum{i in nodes : (j,i) in arcs}(q1[j,i] - q2[j,i]) =  D[j];

s.t. binaryCon{(i,j) in arcs}:
    sum{k in pipes} X[i,j,k] = 1;

s.t. diaCon{(i,j) in arcs}:
    sum{k in pipes} X[i,j,k]*d[k] = dia[i,j];

s.t. con2{(i,j) in arcs}: h[i] - h[j] = (q1[i,j]^1.852- q2[i,j]^1.852) * (0.001^1.852) * omega * L[i,j] / ( (Rough[i,j]^1.852) * (dia[i,j]/1000)^4.87);
