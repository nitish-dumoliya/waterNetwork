...
class WaterNetworkOptimizer:
    ...
    def display_results(self):
        ...
        self.ampl.eval("display {(i,j) in arcs}: q1[i,j];")
        self.ampl.eval("display {(i,j) in arcs}: q2[i,j];")
    ...
    def display_best_results(self,ampl):
        ampl.eval("display {(i,j) in arcs}: q1[i,j];")
        ampl.eval("display {(i,j) in arcs}: q2[i,j];")
        ampl.eval("display l;")
        ampl.eval("display h;")
    ...
    def generate_random_acyclic_from_solution(self):
        ...
        q1 = self.ampl.getVariable('q1').getValues().toDict()
    ...
    def update_model(self):
        ...
        for edge in self.network_graph.edges:
            ...
            if edge in edges_list:
                self.ampl.eval(f"s.t. flow_direction1{i}_{j}: q1[{i},{j}] >=0;")
                self.ampl.eval(f"s.t. flow_direction2{i}_{j}: q2[{i},{j}] >=0;")
                # self.ampl.eval(f"s.t. head_bound_left{i}_{j}: E[{j}]+P[{j}] <= h[{j}];")
                # self.ampl.eval(f"s.t. head_bound_right{i}_{j}: E[{j}] + P[{j}] <= h[{i}];")
            else:
                self.ampl.eval(f"s.t. flow_direction1{i}_{j}: q1[{j},{i}] <=0;")
                self.ampl.eval(f"s.t. flow_direction2{i}_{j}: q2[{j},{i}] <=0;")
    ...
    def iterate_arc(self,best_acyclic_flow, improved, current_cost, l, q1, q2, h, best_arc, super_source_out_arc):
        improved = False
        ...
        for node in self.network_graph.nodes():
            ...
            print("in_degree(", node,")", self.network_graph.in_degree(node))
                        ...
                        if acy_check and in_arc_check:
                            ...
                            if self.solve_result == "solved":
                                if self.total_cost < current_cost:
                                    ...
                                    q1 = self.ampl.getVariable('q1').getValues().to_list()
                                    q2 = self.ampl.getVariable('q2').getValues().to_list()
        ...
        return best_acyclic_flow, improved, current_cost, l, q1, q2, h, best_arc
    ...
    def iterate_acyclic_flows(self):
        ...
        while improved:
            ...
            best_acyclic_flow, improved, current_cost, l, q1, q2, h, best_arc = self.iterate_arc(best_acyclic_flow, improved, current_cost, l, q1, q2, h, best_arc, super_source_out_arc)
    ...
    def solve(self):
        with self.suppress_output():
            ...
            self.ampl.option["solver"] = "knitro"
            ...
            self.ampl.set_option("knitro_options", "outlev = 0 ms_enable = 1 ms_maxsolves = 15 mip_multistart=1")
...
if __name__ == "__main__":
    ...
    optimizer = WaterNetworkOptimizer("../m1BasicNewYork.mod", input_data_file, data_number)
    optimizer.run()
