...
class WaterNetworkSolver:
    ...
    def second_solve(self,model_file, solver_name):
        ...
        self.ampl.option['solver'] = "ipopt"
        # self.ampl.setOption("ipopt_options", "run_sens yes sens_boundcheck no")
        ...
        # self.ampl.eval("display q;")
        # self.ampl.eval("display h;")
        obj_nom = self.ampl.getObjective("total_cost").value()
        print("nominal cost:", obj_nom)
        print("\n=================Estimation of Perturbed Solution with sIPOPT=================\n");
        
        arc_max_dia = {}

        if self.data_number == 6:
            fixarcs = set(self.ampl.getSet("fixarcs").getValues().toList())
        else:
            fixarcs = set()
        ...
        # ampl = ampl
        for d_max in arc_max_dia.values():
            if d_max == 1:
                continue 
            d0 = self.d[d_max]   # ← k is pipe index (string/int) 
            # print(d0)
            ampl.eval(f"""
                let nominal_d[{d_max}].sens_state_0 := {sid};
                let nominal_d[{d_max}].sens_state_1 := {sid};
                let nominal_d[{d_max}].sens_state_value_1 := {0.98 * d0}; 
                let const_d[{d_max}].sens_init_constr := 1;
            """)
            sid += 1 
 
        print("\nSensitivity without bound checking\n")

        ampl.option['solver'] = solver_name
        ampl.setOption("ipopt_options", "run_sens yes sens_boundcheck no outlev 0 warm_start_init_point = yes")
        # ampl.setOption("ipopt_options", "run_sens yes sens_boundcheck no outlev 0")

        ampl.solve()
        l_sens = ampl.getVariable("l").getValues("sens_sol_state_1").toDict()
        # q_sens = ampl1.getVariable("q").getValues("sens_sol_state_1").toDict()
        # h_sens = ampl1.getVariable("h").getValues("sens_sol_state_1").toDict()
        d_sens = ampl.getVariable("nominal_d").getValues("sens_sol_state_1").toDict()
        # eps_sens = self.ampl.getParameter("eps").getValues("sens_sol_state_1").toDict()
 
        obj_nom = ampl.getObjective("total_cost").value()
        # obj_sens = ampl1.getObjective("total_cost").getValues("sens_sol_state_1")
        obj_sens = 0.0
        for (i, j, k), val in l_sens.items():
            obj_sens += val * self.C[k]
        ...
        # print("q:", q_sens)
        print("Nominal cost:", obj_nom)
        print("Sensitivity-predicted cost:", obj_sens)
        
        # print("\nSensitivity with bound checking\n")
        #
        ampl.setOption("ipopt_options", "run_sens yes sens_boundcheck yes outlev 0 warm_start_init_point = yes")
        ampl.solve()
        l_sens = ampl.getVariable("l").getValues("sens_sol_state_1").toDict()
        q_sens = ampl.getVariable("q").getValues("sens_sol_state_1").toDict()
        h_sens = ampl.getVariable("h").getValues("sens_sol_state_1").toDict()
        obj_nom = ampl.getObjective("total_cost").value()
        obj_sens = 0.0
        for (i, j, k), val in l_sens.items():
            obj_sens += val * self.C[k]
