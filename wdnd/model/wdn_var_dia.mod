...
param eps{(i,j) in arcs} := 0.0535*(1/MaxK[i,j])^(0.54);
...
var dvar{arcs, pipes}>=d_min, <=d_max;	            # Flow variable
#****************************************OBJECTIVE****************************************#
...
subject to con2{(i,j) in arcs}: 
    h[i] - h[j]  =  (q[i,j]^3 * (q[i,j]^2 + eps[i,j]^2)^0.426 / (q[i,j]^2 + 0.426*eps[i,j]^2)) * sum{k in pipes}(omega * l[i,j,k] / (R[k]^1.852 * dvar[i,j,k]^4.87));
