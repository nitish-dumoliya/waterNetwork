...
class WaterNetworkOptimizer:
    ...
    def reduced_nlp_model(self, iteration, l_cvx, q_cvx, h_cvx, l, q, h, z_star, sorted_arcs, l_points, q_points, dual_dict):
        ...
        # for (u,v) in self.arcs:
        #     for k in self.pipes:
        #         ampl_cvxnlp.eval(f"""subject to TR_l_lower_{u}_{v}_{k}: l[{u},{v},{k}] >= {l[u,v,k] - self.eta*self.L[u,v]};""")
        #         ampl_cvxnlp.eval(f"""subject to TR_l_upper_{u}_{v}_{k}: l[{u},{v},{k}] <= {l[u,v,k] + self.eta*self.L[u,v]};""")
    ...
    def local_solution_improvement_heuristic(self):
        ...
        if cvx_nlp.solve_result == "solved":
            ...
            print("z_star:", self.z_star, "solve_time:", solve_time, "solve_result:", cvx_nlp.solve_result)
            ...
            if self.solve_result == "solved":
                ...
                if self.total_cost < self.current_cost:
                    ...
                    print(f"{str((i, j)):<10}"
                        ...
                        f"{round(time.time() - self.start_time, 2)}s")
                    # ampl.eval("display q;")
                    # if (self.total_cost - self.current_cost)/self.total_cost <= 1e-4:
                        # self.iteration += 1
                        # self.visited_arc_reverse = []
