...
minimize total_cost : sum{(i,j) in arcs} sum{k in pipes} (omega * l[i,j,k] / ( (R[k]^1.852) * (d[k])^4.87))  * (((q1[i,j])^(2.852))/2.852 + ((q2[i,j])^(2.852))/2.852  ) + sum{u in Source} E[u] * (sum{(i,j) in arcs} (q1[i,j]- q2[i,j])) ; # sum{u in Source} E[u] * (sum{(i,u) in arcs} q[i,u] - sum{(j,u) in arcs} q[j,u]);
...
#s.t. flow{(i,j) in arcs}: q[i,j] = q1[i,j] - q2[i,j];
#*******************************************************************************************#
