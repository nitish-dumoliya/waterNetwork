...
class WaterNetworkOptimizer:
    ...
    def multistart(self, inarc, current_cost, best_acyclic_flow, improved, super_source_out_arc, iteration):
        ...
        for start in range(num_starts): 
            ...
            self.ampl.set_option("omit_zero_rows", "1")
            self.ampl.set_option("omit_zero_cols", "1")
    ...
    def iterate_branching_on_q(self, eps=0, max_branch_per_iter=None):
        ...
        for edge in cand_arcs:
            ...
            for sense, rhs, cname in branches:
                ...
                ampl.option["ipopt_options"] = f"outlev = 0 expect_infeasible_problem = no bound_relax_factor = 0 tol = 1e-9 bound_push = {self.bound_push} bound_frac = {self.bound_frac} warm_start_init_point = yes halt_on_ampl_error = yes mu_strategy = adaptive recalc_y = yes"
                ampl.option["presolve_eps"] = "6.82e-14"
                ampl.option['presolve'] = 1
    ...
    def cvx_nlp_model(self, l_cvx):
        ...
        if self.data_number==5:
            ...
            ampl_cvxnlp.read("modified_cvxnlp.mod")
        ...
        # ampl_cvxnlp.eval(f"""subject to con3_l_:sum {{(i,j) in arcs}} sum {{k in pipes}} C[k] * l[i,j,k] <= {self.current_cost};""")
        # alpha = 0 
        ampl_cvxnlp.eval("""param l_ref {arcs, pipes};""")
        # key: (u, v, k) → value
        # l_ref_data = {(u, v, k): self.l[u, v, k] for (u, v) in self.arcs for k in self.pipes}
        #ampl_cvxnlp.setData(l_ref_data, "l_ref")
        for (u,v,k), val in l_cvx.items():
            ampl_cvxnlp.param["l_ref"][u, v, k] = val
        #for (u,v) in self.arcs:
            # a = sum(10.67*self.l[u,v,k]/(self.R[k]**1.852 * self.d[k]**4.87) for k in self.pipes)
            # alpha += a
            # ampl_cvxnlp.eval(f"""subject to con3_{u}_{v}: x[{u},{v}] >= {sum(10.67*self.l[u,v,k]/(self.R[k]**1.852 * self.d[k]**4.87) for k in self.pipes)};""")
        #ampl_cvxnlp.eval("""subject to con10 { (u,v) in arcs }: sum { k in pipes } (l[u,v,k] - l_ref[u,v,k]) = L[u,v];""")
        # ampl_cvxnlp.eval(f"""subject to con3__: sum{{(i,j) in arcs}} x[i,j] >= {alpha};""")

        ampl_cvxnlp.option['solver'] = "ipopt" 

        ampl_cvxnlp.option["ipopt_options"] = f"outlev = 0 expect_infeasible_problem = no bound_relax_factor = 0 tol = 1e-9 bound_push = {self.bound_push} bound_frac = {self.bound_frac} warm_start_init_point = no halt_on_ampl_error = yes"
        # ampl_cvxnlp.option["ipopt_options"] = f"outlev = 0 expect_infeasible_problem = no  bound_relax_factor=0 warm_start_init_point = no halt_on_ampl_error = yes"
        #with self.suppress_output():
        ampl_cvxnlp.option["presolve_eps"]= "7.19e-13"
    ...
    def diameter_reduction(self):
        ...
        ampl_cvxnlp = self.cvx_nlp_model(self.l)
        # with self.suppress_output():
        ampl_cvxnlp.solve()
        l_cvx = ampl_cvxnlp.getVariable('l').getValues().to_dict()
        q_cvx = ampl_cvxnlp.getVariable('q').getValues().to_dict()
        # print(l_cvx)
        # for idx, val in l_cvx.items():
        #     if val>1e-3:
        #         print(idx, val)
        # ampl_cvxnlp.eval("display l;")
        # ampl_cvxnlp.eval("display sum{(i,j) in arcs} sum{k in pipes} C[k]*l[i,j,k];")
        cvx_total_cost = 0 
        for (a,b) in self.arcs:
            for c in self.pipes:
                cvx_total_cost = cvx_total_cost + l_cvx[a,b,c]*self.C[c]
        print("cvx_total_cost:", cvx_total_cost)
        solve_time = ampl_cvxnlp.get_value('_solve_elapsed_time')
        print("cvx_solve_time:", solve_time)
        
        while ampl_cvxnlp.solve_result != "solved":
            ampl_cvxnlp = self.cvx_nlp_model(l_cvx)
            ampl_cvxnlp.eval("""subject to con10 { (u,v) in arcs }: sum { k in pipes } (l[u,v,k] - l_ref[u,v,k]) >= L[u,v];""")
            # with self.suppress_output():
            ampl_cvxnlp.solve()
            l_cvx = ampl_cvxnlp.getVariable('l').getValues().to_dict()
        ...
        if ampl_cvxnlp.solve_result == "solved":
            ampl = AMPL()
            ampl.reset()
            if self.data_number==5:
                ampl.read("newyork_model.mod")
            ...
            # for key, val in l_cvx.items():
            #     u,v,k = key
            #     # if (key[0], key[1]) != (i,j):
            #     ampl.eval(f"subject to con9__{u}_{v}_{k}: l[{u},{v},{k}] = {l_cvx[u,v,k]};")
            
            for (u,v) in self.arcs:
                # if (u,v) != (i,j):
                #ampl.eval(f"s.t. arc_dia{u}_{v}: sum{{k in pipes: k <=  {arc_max_dia[u,v]+1}}} l[{u},{v},k] = L[{u},{v}];")
                ampl.eval(f"s.t. arc_flow{u}_{v}: q[{u},{v}] = {q_cvx[u,v]};")
            
            ampl.option['solver'] = "ipopt" 

            ampl.option["ipopt_options"] = f"outlev = 0 expect_infeasible_problem = yes bound_relax_factor = 0 tol = 1e-9 bound_push = {self.bound_push} bound_frac = {self.bound_frac} warm_start_init_point = no halt_on_ampl_error = yes"
            #ampl.option["ipopt_options"] = f"outlev = 0 expect_infeasible_problem = no  bound_relax_factor=0 warm_start_init_point = yes halt_on_ampl_error = yes"
            #with self.suppress_output():
            ampl.option["presolve_eps"]= "7.19e-13"
            ...
            if ampl.solve_result == "solved":
                ...
                if total_cost < self.current_cost:
                    ...
                    print("----- Primal Loop Started --------")
                    # ampl.solve_result = "infeasible"
                    while ampl.solve_result == "solved":
                        ampl = self.cvx_nlp_model(l_cvx)
                        ampl.eval("""subject to con10 { (u,v) in arcs }: sum { k in pipes } (l[u,v,k] - l_ref[u,v,k]) >= L[u,v];""")
                        # with self.suppress_output():
                        ampl.solve()
                        l_cvx = ampl.getVariable('l').getValues().to_dict()
                        q_cvx = ampl.getVariable('q').getValues().to_dict()
                        cvx_total_cost = 0 
                        for (a,b) in self.arcs:
                            for c in self.pipes:
                                cvx_total_cost = cvx_total_cost + l_cvx[a,b,c]*self.C[c]
                ...
                while ampl.solve_result != "solved":
                    ampl = self.cvx_nlp_model(l_cvx)
                    # ampl.eval("""subject to con10 { (u,v) in arcs }: sum { k in pipes } (l[u,v,k] - l_ref[u,v,k]) >= L[u,v];""")
                    ampl.eval("""subject to con10 {(i,j) in arcs}: L[i, j] - (sum {k in pipes: l_ref[i,j,k]>1e-3} l[i,j,k]) + (sum {k in pipes: l_ref[i,j,k]=0}l[i,j,k]) = L[i,j];""")
                    # with self.suppress_output():
                    ampl.solve()
                    l_cvx = ampl.getVariable('l').getValues().to_dict()
                    q_cvx = ampl.getVariable('q').getValues().to_dict()
        ...
        #     self.all_duals[con_name] = val.getValues()
        # sorted_arcs = []
        # dual_dict = self.all_duals["con9"].to_dict()
        # # print("dual_value:", dual_dict)
        # sorted_duals = dict(sorted(dual_dict.items(), key=lambda kv: abs(kv[1]), reverse=True))
        # # print("dual_value:", sorted_duals)
        # sorted_arcs = list(sorted_duals.keys())
        # sorted_arcs = [arc for arc in sorted_arcs if arc not in self.visited_arc]
        # # print("sorted_arcs:", self.visited_arc)
        # # print("\nsorted_arcs:", sorted_arcs)
        ...
        # for (i,j) in sorted_arcs:
            ...
            # for k in self.pipes:
            #     if k>=arc_max_dia[i,j]:
            #         ampl_cvxnlp.eval(f"subject to con3__{i}_{j}_{k}: l[{i},{j},{k}] = 0;")
            #
            # ampl_cvxnlp.eval(f"""subject to con3_l_:sum {{(i,j) in arcs}} sum {{k in pipes}} C[k] * l[i,j,k] <= {self.current_cost};""")
            #
            # ampl_cvxnlp.option['solver'] = "ipopt" 
            ...
            #         cvx_total_cost = cvx_total_cost + l_cvx[a,b,c]*self.C[c]
            # # print("cvx_total_cost:", cvx_total_cost)
            # cost_cvx = ampl_cvxnlp.getObjective("total_cost").value()
            # solve_time = ampl_cvxnlp.get_value('_solve_elapsed_time')
            # print(f"\n{str((i,j)):<10}"
            #                 f"{self.format_indian_number(round(self.current_cost)):<14}"
            #                 f"{round(cost_cvx,4):<14}"
            #                 f"{(str(round(solve_time, 2)) + 's'):<12}"
            #                 f"{ampl_cvxnlp.solve_result:<14}{'':<10}"
            #                 f"{round(time.time() - self.start_time, 2)}s")
            ...
            # ampl.option['solver'] = "ipopt" 
            #
            # ampl.option["ipopt_options"] = f"outlev = 0 expect_infeasible_problem = yes bound_relax_factor = 0 tol = 1e-9 bound_push = {self.bound_push} bound_frac = {self.bound_frac} warm_start_init_point = yes halt_on_ampl_error = yes"
            # #ampl.option["ipopt_options"] = f"outlev = 0 expect_infeasible_problem = no  bound_relax_factor=0 warm_start_init_point = yes halt_on_ampl_error = yes"
            # #with self.suppress_output():
            # ampl.option["presolve_eps"]= "7.19e-13"
            #
    ...
    # ---------------------------------------------------
    # SOLVE NLP WITH FIXED q
    # ---------------------------------------------------
    def solve_nlp_with_fixed_q(self, q_fix, l_fix):
        ampl = AMPL()
        ampl.reset()
        ...
        for (i, j), val in q_fix.items():
            ampl.eval(f"s.t. fix_q_{i}_{j}: q[{i},{j}] = {val};")
        # for (i, j, k), val in l_fix.items():
        #     ampl.eval(f"s.t. fix_l_{i}_{j}_{k}: l[{i},{j}, {k}] = {val};")


        ampl.option['solver'] = "ipopt"
        # ampl.option["ipopt_options"] = (
        #     "outlev=0 expect_infeasible_problem=yes "
        #     f"bound_push={self.bound_push} bound_frac={self.bound_frac} "
        #     "tol=1e-9"
        # )
        ampl.option["ipopt_options"] = f"outlev = 0 expect_infeasible_problem = yes bound_relax_factor = 0 tol = 1e-9 bound_push = {self.bound_push} bound_frac = {self.bound_frac} warm_start_init_point = no halt_on_ampl_error = yes"
        ampl.option["presolve_eps"]= "7.19e-8"
    ...
    # BUILD & SOLVE CVX–NLP
    # ---------------------------------------------------
    def solve_cvx_nlp(self, l_ref, use_partition=False):
        ampl = AMPL()
        ampl.reset()

        # ampl.read("modified_cvxnlp.mod")
        ampl.read(self.model_file)
        ampl.read_data(self.data_file)
        for (x, y, k), val in self.l.items():
            ampl.eval(f'let l[{x},{y},{k}] := {val};')
        ...
        ampl_tuples = [tuple((u,v)) for (u,v) in list(self.fix_arc_set)]
        print("fix_arcs:", ampl_tuples)
        ampl.eval(f'set fix_arcs within {{i in nodes, j in nodes: i != j}};')
        ampl.set[f'fix_arcs'] = ampl_tuples

        # ampl.eval(f"""subject to con3_l_:sum {{(i,j) in arcs}} sum {{k in pipes}} C[k] * l[i,j,k] <= {self.current_cost};""")
        for id in range(len(self.L_ref)):
            ampl.eval(f"param l_ref{id} {{arcs,pipes}};") 
            for (i, j, k), val in self.L_ref[id].items():
                ampl.param[f"l_ref{id}"][i, j, k] = val

        if use_partition:
            # ampl.eval("""
            # subject to con_partition {(i,j) in arcs}:
            #     L[i,j] - (sum {k in pipes: l_ref[i,j,k] > 1e-6} l[i,j,k]) +
            #     (sum {k in pipes: l_ref[i,j,k] <= 1e-6} l[i,j,k]) = L[i,j];
            # """)
            # ampl.eval("""subject to con10 {(i,j) in arcs diff fix_arcs}: L[i, j] - (sum {k in pipes: l_ref[i,j,k]>1e-6} l[i,j,k]) + (sum {k in pipes: l_ref[i,j,k]<=1e-6}l[i,j,k]) = L[i,j];""")
            for id in range(len(self.L_ref)):
                ampl.eval(f"subject to con10_{id} {{(i,j) in arcs diff fix_arcs}}: 1 - (sum {{k in pipes: l_ref{id}[i,j,k]>1e-3}} l[i,j,k]/L[i,j]) + (sum {{k in pipes: l_ref{id}[i,j,k]<=1e-3}} l[i,j,k]/L[i,j])  >= 1;")
            # ampl.eval("""subject to con10 {(i,j) in arcs diff fix_arcs}: (sum {k in pipes} abs(l[i,j,k] - l_ref[i,j,k])) <= 2*L[i,j];""")

        ampl.option['solver'] = "ipopt"
        # ampl.option["ipopt_options"] = (
        #     "outlev=0 expect_infeasible_problem=no "
        #     f"bound_push={self.bound_push} bound_frac={self.bound_frac} "
        #     "tol=1e-9"
        # )
        ampl.option["ipopt_options"] = f"outlev = 0 expect_infeasible_problem = yes bound_relax_factor = 0 tol = 1e-9 bound_push = {self.bound_push} bound_frac = {self.bound_frac} warm_start_init_point = yes halt_on_ampl_error = yes"
        ampl.option["presolve_eps"]= "7.19e-7"
    ...
    # ---------------------------------------------------
    # MAIN WDND HEURISTIC (YOUR PSEUDOCODE)
    # ---------------------------------------------------
    def wdnd_heuristic(self):

        print("\n========== WDND CVX–NLP HEURISTIC ==========\n")

        # STEP 1: Solve NLP
        # self.solve_nlp()
        # if self.ampl.solve_result != "solved":
        #     print("Initial NLP infeasible. Abort.")
        #     return
        #
        z_star = self.current_cost
        l_star = self.l
        q_star = self.q
        h_star = self.h
        #
        ...
        while improved or use_partition:
            improved = False

            # STEP 2 / STEP 4: Solve CVX–NLP
            ampl_nlp = self.solve_cvx_nlp(l_star, use_partition)

            if ampl_nlp.solve_result != "solved":
                print("CVX infeasible → activate partition constraint")
                # improved = True
                # l_star = ampl_cvx.getVariable("l").getValues().to_dict()
                use_partition = False
                continue

            # l_star = ampl_nlp.getVariable("l").getValues().to_dict()
            # q_cvx = ampl_nlp.getVariable("q").getValues().to_dict()
            # self.L_ref.append((l_star))

            # STEP 3: Fix q and solve NLP
            # ampl_nlp = self.solve_nlp_with_fixed_q(q_cvx, l_star)

            # if ampl_nlp.solve_result != "solved":
            #     print("Fixed-q NLP infeasible → tighten CVX")
            #     # improved = True
            #     # l_star = ampl_nlp.getVariable("l").getValues().to_dict()
            #     use_partition = True
            #     continue
            ...
            l_star = ampl_nlp.getVariable("l").getValues().to_dict()
            self.L_ref.append((l_star))
            z = ampl_nlp.getObjective("total_cost").value()
            solve_time = ampl_nlp.get_value('_solve_elapsed_time')

            if z < z_star:
                #print(f"Improved: {z_star:.3f} → {z:.3f}")
                print(f"{self.format_indian_number(round(self.current_cost)):<14}"
                        f"{self.format_indian_number(round(z)):<14}"
                        f"{(str(round(solve_time, 2)) + 's'):<12}"
                        f"{ampl_nlp.solve_result:<14}{'Yes':<10}"
                        f"{round(time.time() - self.start_time, 2)}s")
                ...
                l_star = ampl_nlp.getVariable("l").getValues().to_dict()
                q_star = ampl_nlp.getVariable("q").getValues().to_dict()
                h_star = ampl_nlp.getVariable("h").getValues().to_dict()
                ...
                print(f"{self.format_indian_number(round(self.current_cost)):<14}"
                        f"{self.format_indian_number(round(z)):<14}"
                        f"{(str(round(solve_time, 2)) + 's'):<12}"
                        f"{ampl_nlp.solve_result:<14}{'No':<10}"
                        f"{round(time.time() - self.start_time, 2)}s")

                print("No improvement → partition CVX")
                # improved = True
                use_partition = True

        print("\n========== HEURISTIC FINISHED ==========")
        print(f"Final best cost: {self.current_cost}")
        print(f"NLP solves: {self.number_of_nlp}")
    ...
    def iterate_acyclic_flows(self):
        ...
        alpha = 1.852  # Hazen–Williams exponent
    
        # --------------------------------------------------
        # Nodes with indegree >= 2
        # --------------------------------------------------
        self.indegree_2_or_more = [
            node for node, indeg in self.network_graph.in_degree()
            if indeg >= 2
        ]
        ...
        sorted_all_arcs = [
            ...
            and arc not in self.visited_arc_reverse
        ]
    
        # --------------------------------------------------
        # Extract headloss duals (con2)
        # --------------------------------------------------
        dual_dict = {}
        for con_name, dual_values in self.all_duals.items():
            if con_name == "con2":
                d = dual_values.to_dict()
                dual_dict = {k: v for k, v in d.items() if k in sorted_all_arcs}
    
        # --------------------------------------------------
        # DGSM SCORE: |dual| * |q|^alpha
        # --------------------------------------------------
        dgsm_score = {}
        for (i, j), dual_val in dual_dict.items():
            if self.q[i, j] != 0:
                dgsm_score[(i, j)] = abs(dual_val) * (abs(self.q[i, j]) ** alpha)
        ...
        print("DGSM sorted arcs:", sorted_by_abs_dual)
        print("----------------------------------------------------------------------------------------")
        print(f"{'Arc':<10}{'C_Best_Sol':<14}{'New_Sol':<14}"
        ...
        # ==================================================
        # MAIN LOOP (ALWAYS USE UPDATED LIST)
        # ==================================================
        while sorted_by_abs_dual:
    
            edge = sorted_by_abs_dual.pop(0)
            (u, v) = edge
            self.visited_arc_reverse.append(edge)
            ...
            # ---------------- force reversal ----------------
            if self.q[u, v] >= 0:
                ampl.eval(f"s.t. flow_direction_{u}_{v}: q[{u},{v}] <= 0;")
            else:
                ampl.eval(f"s.t. flow_direction_{u}_{v}: q[{u},{v}] >= 0;")
            ...
            # ---------------- detect reversed arcs ----------------
            self.reversed_arcs = [
                (i, j) for (i, j) in self.arcs
                if self.q[i, j] * q_new[i, j] < 0
            ]
    
            # ---------------- update candidate list ----------------
            # sorted_by_abs_dual = [
            #     arc for arc in sorted_by_abs_dual
    ...
    def diameter_reduction_old(self):
        improved = False
        arc_max_dia = {}
        if self.data_number == 6:
            self.fixarcs = self.ampl.getSet('fixarcs')
        ...
        for (i,j) in sorted_arcs:
            ...
            # ampl.eval(f"subject to con3_{i}_{j}: sum{{k in pipes: k <=  {arc_max_dia[i,j]-1}}} l[{i},{j},k] = L[{i},{j}];")
            # ampl.eval(f"subject to con3_l{i}_{j}_{arc_max_dia[i,j]}: l[{i},{j},{arc_max_dia[i,j]}] <= {self.l[i,j,arc_max_dia[i,j]]} - 1e-0;"
            # ampl.eval(f"subject to con3_l{i}_{j}: sum {{k in pipes}} (omega * l[{i},{j},k] / (R[k]^1.852 * d[k]^4.87)) >= {sum(10.67 * self.l[i,j,k]/(self.R[k]**1.852 * self.d[k]**4.87) for k in self.pipes)};")
            # ampl.eval(f"subject to con3_l{i}_{j}: sum {{k in pipes}} C[k]*l[{i},{j},k] <= {self.C[arc_max_dia[i,j]] * self.L[i,j]};")
            ...
            ampl.eval("""param l_ref {arcs, pipes};""")
            # key: (u, v, k) → value
            # l_ref_data = {(u, v, k): self.l[u, v, k] for (u, v) in self.arcs for k in self.pipes}
            #ampl_cvxnlp.setData(l_ref_data, "l_ref")
            for (u,v,k), val in self.l.items():
                ampl.param["l_ref"][u, v, k] = val
            #for (u,v) in self.arcs:
                # a = sum(10.67*self.l[u,v,k]/(self.R[k]**1.852 * self.d[k]**4.87) for k in self.pipes)
                # alpha += a
                # ampl_cvxnlp.eval(f"""subject to con3_{u}_{v}: x[{u},{v}] >= {sum(10.67*self.l[u,v,k]/(self.R[k]**1.852 * self.d[k]**4.87) for k in self.pipes)};""")
            #ampl.eval(f"subject to con10_{i}_{j}: L[{i}, {j}] - (sum {{k in pipes: l_ref[{i},{j},k]>1e-3}}l[{i},{j},k]) + (sum {{k in pipes: l_ref[{i},{j},k]=0}}l[{i},{j},k]) = L[{i},{j}];")
        
            # ampl.eval(f"subject to con10_{i}_{j}: 1 - (sum {{k in pipes: l_ref[{i},{j},k]>1e-3}} l[{i},{j},k]/L[{i},{j}]) + (sum {{k in pipes: l_ref[{i},{j},k]<=1e-3}} l[{i},{j},k]/L[{i},{j}])  >= 1;")
            # ampl.eval(f"s.t. flow_value{i}_{j}: q[{i}, {j}]>=abs({self.q[i,j]});")
            # if self.h[i] - self.h[j]>= 0:
            #     ampl.eval(f"s.t. headloss1{i}_{j}: h[{i}]- h[{j}]>={self.h[i] - self.h[j]} + 1e-0;")
            # else:
            ...
            #ampl.option["ipopt_options"] = f"outlev = 0 expect_infeasible_problem = no  bound_relax_factor=0 warm_start_init_point = yes halt_on_ampl_error = yes"
            #with self.suppress_output():
            # ampl.set_option("ipopt_options", "run_sens yes sens_boundcheck no")
            ampl.option["presolve_eps"]= "9.23e-11"
    ...
    def diameter_reduction_old2(self):
        improved = False
        arc_max_dia = {}
        if self.data_number == 6:
            self.fixarcs = self.ampl.getSet('fixarcs')
        ...
        sorted_arcs = self.sorted_arc_dia_red()       
        # sorted_arcs = [sorted_arcs[0]]
        sorted_arcs = dict(sorted(sorted_arcs.items(), key=lambda kv: abs(kv[1]), reverse=True))
    ...
    def iterate_acyclic_flows(self):
        ...
        print("sorted_arcs:", sorted_by_abs_dual)
        print("----------------------------------------------------------------------------------------")
        print(f"{'Arc':<10}{'C_Best_Sol':<14}{'New_Sol':<14}"
              f"{'Solve_Time':<12}{'Solve_Result':<14}{'Improved':<10}{'Time':<12}")
        print("----------------------------------------------------------------------------------------")
        ...
        while sorted_by_abs_dual:
            ...
            # -------- enforce reversal on selected arc --------
            if self.q[u, v] >= 0:
                ampl.eval(f"s.t. flow_direction_{u}_{v}: q[{u},{v}] <= 0;")
            else:
                ampl.eval(f"s.t. flow_direction_{u}_{v}: q[{u},{v}] >= 0;")
            ...
            # for (i,j) in self.arcs:
                # ampl.eval(f"s.t. flow_bound_reduction_{i}_{j}: abs(q[{i},{j}] - {self.q[i,j]}) <= 1;")
    ...
    def iterate_acyclic_flows_old2(self):
        ...
        sorted_by_abs_dual = dict(sorted(dual_dict.items(), key=lambda kv: abs(kv[1]), reverse=True))
        sorted_by_abs_dual = list(sorted_by_abs_dual.keys())
        # sorted_by_abs_dual = [arc for arc in sorted_by_abs_dual if arc not in self.reversed_arcs]
        print("sorted_arcs: ", sorted_by_abs_dual)
        ...
        for edge in sorted_by_abs_dual:
            self.visited_arc_reverse.append(edge)
            (u,v) = edge
            # self.load_model()
            ampl = AMPL()
            ...
            if self.solve_result == "solved":
                ...
                self.reversed_arcs = []                
                for (i, j) in self.arcs:
                    if self.q[i, j] * q[i, j] < 0:
                        self.reversed_arcs.append((i, j))
    ...
    def iterate_acyclic_flows_old(self):
        ...
        for edge in sorted_by_abs_dual.keys():
            ...
            if improved:
                self.iteration += 1
                self.iterate_acyclic_flows_old()
                break
        # print("----------------------------------------------------------------------------------------")

    def no_good_cut_approach(self):
        print("\nStarting exclusion-based global search (AMPL API)\n")
        # Add constraints only once
        # if not hasattr(self, "exclusion_added"):
        #     self.add_exclusion_constraints()
        #     self.exclusion_added = True
        eps_schedule = [1e-4, 1e-3, 1e-2, 5e-2, 1e-1, 0.2, 0.5, 1.0, 2, 3]
        best_cost = self.current_cost
        best_l = copy.deepcopy(self.l)
        best_q = copy.deepcopy(self.q)
        best_h = copy.deepcopy(self.h)
        for eps in eps_schedule:
            print(f"\n--- Global search iteration {self.iteration}, eps = {eps} ---")
            ampl = AMPL()
            ampl.reset()
            if self.data_number==5:
                ampl.read("newyork_model.mod")
            ...
            ampl.eval("""
                    param l_bar{arcs,pipes} default 0;
                    param q_bar{arcs} default 0;
                    param h_bar{nodes} default 0;
                    param eps_cut >= 0;
                    var d_pos{arcs,pipes} >= 0;
                    var d_neg{arcs,pipes} >= 0;
                    """)

            ampl.getParameter("eps_cut").set(eps)

            l_bar = ampl.getParameter("l_bar")
            q_bar = ampl.getParameter("q_bar")
            h_bar = ampl.getParameter("h_bar")
            ...
            for (i, j), val in self.q.items():
                q_bar[(i, j)] = val
            for k, val in self.h.items():
                h_bar[k] = val

            # ampl.eval("""subject to dev_def{(i,j) in arcs, k in pipes}: l[i,j,k] - l_bar[i,j,k] = d_pos[i,j,k] - d_neg[i,j,k];"""
            ampl.eval("""set ActivePipes := {(i,j,k) in arcs cross pipes: l_bar[i,j,k] > 1e-3};""")
            # ampl.eval("""subject to no_good_cut: sum{(i,j,k) in ActivePipes} (d_pos[i,j,k] + d_neg[i,j,k]) >= eps_cut;""")
            ampl.eval("""subject to no_good_cut1: sum{(i,j,k) in ActivePipes} abs(l[i,j,k] - l_bar[i,j,k])  >= eps_cut;""")
            ampl.eval("""subject to no_good_cut2: sum{(i,j) in arcs} abs(q[i,j] - q_bar[i,j])  >= eps_cut;""")
            ampl.eval("""subject to no_good_cut3: sum{k in nodes diff Source} abs(h[k] - h_bar[k])  >= eps_cut;""")
            # ------------------------------------
            # Update incumbent solution
            # ------------------------------------
            # ------------------------------------
            # Solve diversified NLP
            # ------------------------------------
            ampl.option["solver"] = "ipopt"
            ampl.option["ipopt_options"] = f"outlev = 0 expect_infeasible_problem = no bound_relax_factor = 0 tol = 1e-9 bound_push = {self.bound_push} bound_frac = {self.bound_frac} warm_start_init_point = no halt_on_ampl_error = yes mu_strategy = adaptive recalc_y = yes"
            with self.suppress_output():
                ampl.solve()
            ...
            if self.solve_result != "solved":
                print("IPOPT failed — skipping epsilon")
                self.iteration += 1
                continue
            new_cost = self.total_cost
            print("Objective:", new_cost)
            # ------------------------------------
            # Accept improvement
            # ------------------------------------
            if new_cost < best_cost - 1e-6:
                print("✔ Improved local optimum found")
                best_cost = new_cost
                best_l = self.ampl.getVariable("l").getValues().to_dict()
                best_q = self.ampl.getVariable("q").getValues().to_dict()
                best_h = self.ampl.getVariable("h").getValues().to_dict()
                self.current_cost = best_cost
                self.l = best_l
                self.q = best_q
                self.h = best_h
                print("Resetting epsilon around new basin")
                self.iteration += 1
                continue
            else:
        ...
        # ------------------------------------
        print("\nRestoring best solution after global search")
        self.current_cost = best_cost
        self.l = best_l
        self.q = best_q
        self.h = best_h
        print("Best objective:", self.current_cost)
    ...
    def solve(self):
        ...
        # self.ampl.set_option("suffix sens_state_0, IN;")
        # self.ampl.set_option("suffix sens_state_1, IN;")
        # self.ampl.set_option("suffix sens_state_value_1, IN;")
        # self.ampl.set_option("suffix sens_sol_state_1, OUT;")
        # self.ampl.set_option("suffix sens_init_constr, IN;")


        # self.ampl.set_option("ipopt_options", "run_sens yes sens_boundcheck no")
    ...
    def solve1(self):
        with self.suppress_output():
            ...
            self.ampl.set_option("ipopt_options", f"outlev = 0 expect_infeasible_problem = no bound_relax_factor=0 tol = 1e-9 bound_push = {self.bound_push} bound_frac = {self.bound_frac} warm_start_init_point = no halt_on_ampl_error = yes")   #max_iter = 1000
    ...
    def run(self):
        ...
        print("\n--------------------------No Good Cut Approach-------------------------------")
        self.iteration = 1
        self.no_good_cut_approach()
